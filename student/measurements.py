...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        ############
        sens_pos = np.ones((4,1))
        sens_pos[0:3] = x[0:3]
        sens_pos = self.veh_to_sens * sens_pos
        in_fov = False
        if sens_pos[0]>0:
            alpha = np.arctan(sens_pos[1]/sens_pos[0])
            in_fov = True if abs(alpha) < self.fov[1] else False
    ...
    def get_hx(self, x):    
        ...
        if self.name == 'lidar':
            ...
            if pos_sens[0] == 0:
                raise NameError("Cannot compute Jacobian")
            else:
                hx = np.zeros((2,1))
